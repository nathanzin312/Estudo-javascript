...
function valorPadro2(a = 1, b = 1, c = 1, d = 1) {
  //se um parametro nao receber valor ele vai valer por padrao 1
  return a + b + c + d;
}
...
//exercicio segunda lista
function media2(obj) {
  let result = new Object();
  let calc = 0;
  for (let i in obj.notas) {
    calc += obj.notas[i];
  }
  let calc1 = 0;
  calc1 = calc / obj.notas.length;
...
let alunos = {
  nome: "Joao",
  notas: [8, 7.6, 8.9, 6], // média 7.625
};
console.log(media2(alunos));
let m = 0;
let w = 0;
function maior(n, i) {
  if (n > m) {
...
console.log([12, 16, 1, 5].forEach(maior));

function l(array, tam) {
  let result = [];
  for (i of array) {
    if (String(i).length === tam) {
      result.push(i);
    }
  }
  return result;
...
function anosemdias(idadeAnos) {
  return idadeAnos * 365;
}

console.log(anosemdias(3));

function salario(horas, valorHora) {
  return "O funcinario tem salario igual a: " + horas * valorHora;
}

let salario2 = (horas, valorHora) =>
  "O funcinario tem salario igual a: " + horas * valorHora;

console.log(salario2(150, 40.5));
